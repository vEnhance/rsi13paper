...
\begin{proof}
	...
	Let us now compose $M_a \defeq \MM(\ul a)$ and $M_b \defeq \MM(\ul b)$.  Two components (one from $M_a$ and one from $M_b$) are said to \emph{feel} each other $k$ times if they have $k$ antennae in common.  Notice that if any two components feel each other at least twice, then there exist two adjacent antennae, whence the composition of the map is zero because the two antenna combine to form an empty face.  Furthermore, it is easy to see that any faces must be formed in this manner.  In combination with the claim above, this implies the first case of the theorem.
...
We turn our attention to the case of $\ul r = \AA_n$.  Let us make a few definitions before proceeding.

Consider a composition of two maps $\MM_{\AA_n}( \ul a, \ul b)$ and the associated diagram $D$.  Let us make a few convenient definitions for the graphical structures.
\begin{definition*}
	A \emph{pocket} is a face, and its \emph{contents} are the connected components inside that face.
\end{definition*}
\begin{definition*}
	A \emph{fence} is a contiguous path which runs from the top of the boundary to the bottom of the boundary (i.e. paths between the labelled vertices).  The diagram is divided by these fences into \emph{pastures}, which will be numbered from left to right as $0$, $1$, \dots, $M$.
\end{definition*}
