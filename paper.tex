...
% TODO restate all theorems in appendix
...
In 1979, Kazhdan and Lusztig \cite{ref:KaLu1,ref:KaLu2} gave a categorification of $\HH$, using a somewhat complicated construction\footnote{The technique involved using something called \emph{perverse sheaves on the flag variety}.}.  The result became known as the Hecke category.
...
Now the morphisms of Bott-Samelson bimodules admit a combinatorial description.  In particular, Elias and Williamson \cite{ref:gr4all}, building off earlier works such as Elias and Khovanov \cite{ref:EKh}, give a presentation for the category of these morphisms in terms of colored planar diagrammatics (i.e. colored planar graphs). 

This paper investigates some of the properties of these diagrammatics from a combinatorial perspective.   In this paper, we consider only the special case where $W$ is an infinite dihedral group with generators $s$ and $t$ (as opposed to an arbitrary Coxeter group $W$).
...
Appendix~\ref{sec:prelim_background} contains the preliminaries for the problem -- namely, the light leaves and the relation between the diagrams and maps.  A minimal set of definitions used in this paper is given in Section~\ref{sec:prelim}.  
...
This motivates us to make the following definition, which we use in Section~\ref{sec:res_alt_compute}.
\begin{definition}
...
\end{ques*}
In this paper, we are interested in computing the polynomial which corresponds to this map.
...
\begin{restatable}[Pasture Lemma]{lemma}{pasture}
	...
	\begin{enumerate}[(i)]
		...
		\ii Pasture $N$ is empty if $T[N] = \AA_n[n]$, and contains a single caterpillar otherwise.  The caterpillar is blue if $\AA_n[n]$ is $s$, and red otherwise.
	\end{enumerate}
	\label{thm:pasture}
\end{restatable}
...
\begin{definition}
	If a nonzero map $M$ with base $\AA_n$ is presented as in Theorem~\ref{thm:alt_struct}, then we call the term $x^{m_x}y^{m_y}\alpha_s^{m_s}\alpha_t^{m_t}$ the \emph{fluff} of $M$, and call the polynomial (either $1$ or $Q_T$) the \emph{face} of $M$. 
\end{definition}
\begin{remark*}
