...
NOTE: If possible, this document should be printed in color.  However, I have left blue lines as solid and red lines as dotted for readability.
...
\[
	...
	\end{cases}.
\]
Let $\BB_n$ denote the set of binary strings of length $n$.
...
\begin{definition*}
	A \emph{bubble} is a bounded face alongside with any components contained within it, which are its \emph{contents}.  The color of the bubble is the color of the edges which form the bounded face.
\end{definition*}
...
\begin{definition*}
	A \emph{caterpillar} is any connected component for which each edge belongs to some bubble (in the caterpillar).  The \emph{size} of the caterpillar is the number of bubbles it is composed of.
\end{definition*}

Notice that every free component can be expressed as a collection of caterpillars joined by edges.  Furthermore, every attached component can be described as a free component attached by edges to a fence.
...
\begin{lemma}[Bubble Lemma]
	Consider a bubble in a full-diagram with base $\AA_n$.  The contents of the bubble are either a barbell or a caterpillar.
\end{lemma}
\begin{corollary}[Caterpillar Corollary]
...
\begin{theorem}[Pasture Theorem]
	Consider a full-diagram with $N+1$ pastures with base $\AA_n$, numbered $0$, $1$, \dots, $N$, where $N \ge 1$.  Then
	\begin{enumerate}[(i)]
		\ii Pastures $1$ to $N-1$ contain no free components and at most one attached component.
		\ii Pasture $N$ contains at most one free component, either a caterpillar or a barbell.
	\end{enumerate}
\end{theorem}

TODO: Work out details of the proofs.
