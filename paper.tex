...
\begin{definition*}[Quantum Numbers]
	For each integer $n \ge 0$, define two polynomials $[n]_x, [n]_y \in \ZZ[x,y]$ recursively by $[0]_x = [0]_y = 0$, $[1]_x = [1]_y = 1$, $[2]_x = x$, $[2]_y = 2$, and
	\begin{align*}
...
\begin{corollary}
	For any binary strings $\ul a$ and $\ul b$ of length $n$
\end{corollary}<++>
...
\begin{definition*}
	We say that two strings $\ul{r_1}$ and $\ul{r_2}$ \emph{agree in $c$} if their last characters are both $c$.
\end{definition*}
\begin{definition*}
...
\begin{proposition}
	Suppose $n \ge 1$ is an integer.  Let $\ul a$ and $\ul b$ be two binary strings of length $n$ which agree in the last bit, which have the same top $T = \Top_{\AA_n} \ul a = \Top_{\AA_n} \ul b$.  Suppose $M_1 = \MM_{\AA_{n-1}} (\ul a', \ul b') \neq 0$.  Then $M = \MM_{\AA_n} (\ul a, \ul b$) is nonzero, and the following recursion holds.
	\begin{enumerate}[(i)]
		\ii If $\AA_n$ and $T$ have the same last character $c$ (so $T \neq \varnothing$)
		\begin{itemize}
			\ii If $\ul a[n] = \ul[b] = 1$, then $M = M_1$.
			\ii If $\ul a[n] = \ul[b] = 0$, then $M = -f / \alpha_k$ where $f$ is the monomial in $M_1$ with the largest factor of $\alpha_k$, with any quantum numbers
		\end{itemize}<++>
	\end{enumerate}<++>
	\label{thm:recurse}
\end{proposition}
