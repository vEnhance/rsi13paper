...
\subsection{An Extended Introduction}
\label{sec:prelim}
...
\end{definition*}
\begin{remark*}
	Notice that any barbell or bubble is a special case of a caterpillar.
\end{remark*}
...
TODO: Write up / prove the formula for number of maps from $A$ to $B$.
...
\begin{theorem}
	Let $\ul r$ be an expression of length $t$, and suppose $R(\ul r) = \AA_n$.
	\begin{enumerate}
		\ii If $n=2k+1$ is odd, then for each $0 \le m \le k$, there are $2^{t-n} \binom{2k}{m+k}$ binary strings $\ul b$ yielding each of $\AA_{2m+1}$, $\AA_{2m}$, $\AA_{2m-1}'$, and $\AA_{2m}'$ as $\Top_{\ul r}(\ul b)$.
		\ii If $n=2k$ is even, then for each $0 \le m \le k-1$, there are $2^{t-n} \binom{2k-1}{m+k}$ binary strings $\ul b$ yielding each of $\AA_{2m+1}$, $\AA_{2m+2}$, $\AA_{2m}'$, and $\AA_{2m+1}'$ as $\Top_{\ul r}(\ul b)$.
	\end{enumerate}
\end{theorem}
...
% We prove theorem \ref{thm:bubble}.
...
\begin{figure}[ht]
	...
	\begin{asy}
		...
		draw(one, (4,0)--(4,h/2)..((4+6)/2.0,h*2)..(6,h/2)--(6,0), t);
		draw(one, (0,0)--(0,h/2)..((0+7)/2.0,h*7)..(7,h/2)--(7,0), s);
		draw(one, (2,0)--(2,h), s);
		dot(one, (2,h), dot_s);
		draw(one, (5,0)--(5,h), s);
		dot(one, (5,h), dot_s);
		...
		picture two;
		draw(two, (1,0)--(1,h/2)..((1+3)/2.0,h*2)..(3,h/2)--(3,0), t);
		draw(two, (4,0)--(4,h/2)..((4+6)/2.0,h*2)..(6,h/2)--(6,0), t);
		draw(two, (0,0)--(0,h/2)..((0+7)/2.0,h*7)..(7,h/2)--(7,0), s);
		draw(two, (2,0)--(2,h), s);
		dot(two, (2,h), dot_s);
		draw(two, (5,0)--(5,h), s);
		dot(two, (5,h), dot_s);
		...
		draw((-1,0)--(8,0));
	\end{asy}
	\caption{The bubble theorem is not true for non-alternating sequences.}
\end{figure}
Consider a bubble $B$ and assume without loss of generality that it is blue.  First, we verify that $B$ is nonempty.  Take any pair of blue anchors on the boundary of $B$.  Then there must be a red point between these two anchors, and it follows that there exists a component of red color inside the bubble, and hence at least one caterpillar.
...
TODO: Finish this.  
% We claim there exists a blue anchor which is not contained inside any bubble.
%Consider the portion of the anchorline which is in the on or inside $B$, and view it as a collection of intervals in $\RR$.  Let us call this set $S \subset \RR$.  For every red bubble, color red the portion of $S$ which is contained inside it or on the boundary, and call this collection $R \subset S$.  Two bubbles are connected if the portion of $S$ they mark touches, even at an endpoint\footnote{But the converse is not true}.  Since there are at least two caterpillars, there must thus be two intervals in $R$, generated by bubbles from different caterpillars, with no red color between them.  (Otherwise, all the red bubbles are connected and are hence part of a single caterpillar.) Since the endpoints of all intervals in $R$ are red anchors, we obtain a blue anchor that is not contained in any red bubble.  

% Because the caterpillars are disjoint and not contained in each other, it is possible to draw a path in the plane such that each caterpillar is on one side of this path.  
...
\begin{proof}[Proof of Theorem \ref{thm:caterpillar}]
	...
	For the inductive step, consider an arbitrary bubble.  Inside it is exactly one caterpillar, say $c$, by the Bubble Theorem.  Because the caterpillar has nesting level $0$, that caterpillar is merely a barbell, and hence is either $\alpha_s$ or $\alpha_t$.
	Applying the barbell-forcing rule of equation \eqref{eq:barbell_break} generates two terms; one of them is zero because it creates a new empty bubble, and one of them is one of $\left\{ x,y \right\}$ times the same diagram with the cycle cut open and the contents removed.  The inductive hypothesis now applies directly, and this completes the induction.

	Now suppose a caterpillar has nesting level $k \ge 1$.  We simply apply the exact same induction on the size as before.  The only modification is that, instead of a barbell, we have a caterpillar with nesting level strictly less than $k$.  However, these caterpillars all evaluate as a polynomial again of the form $x^my^n \alpha_s$ or $x^my^n\alpha_t$, by the outer inductive hypothesis.  So applying equation \eqref{eq:barbell_break} in the same manner completes this induction as well.
\end{proof}
