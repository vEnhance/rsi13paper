...
Further details on the evaluation of these maps are detailed in section \ref{sec:explain_poly_eval}, but before that we need to introduce certain definitions for clarity.
...
\begin{definition*}
	A \emph{bubble} is a bounded face alongside with any components contained within it, which are its \emph{contents}.  
\end{definition*}
...
\section{Constraints on the Diagrammatics in the Alternating Case}
...
\begin{corollary}[Caterpillar Corollary]
	A caterpillar in a full-diagram with base $\AA_n$ evaluates as 
\end{corollary}<++>
