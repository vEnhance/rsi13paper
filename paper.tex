...
Two maps $\MM_{\ul r}(\ul a)$ and $\MM_{\ul r}(\ul b)$ with identical tops may be composed by juxtaposing $\MM_{\ul r}(\ul a)$ with a flipped copy of $\MM_{\ul r}(\ul b)$; an example of such a composition is given in Figure~\ref{fig:example_compose}.  We will denote this product by $\MM_{\ul r}(\ul a, \ul b)$.  If the two half-maps are not compatible -- i.e. $\Top_{\ul r} \ul a \neq \Top_{\ul r} \ul b$ -- then we will simply treat the product as the zero map.  % Then given two binary strings $\ul a$ and $\ul b$, we will let $\MM_{\ul r}(\ul a, \ul b)$ denote the product of $\MM_{\ul r}(\ul a)$ and $\MM_{\ul r}(\ul b)$ if $\Top_{\ul r} \ul a = \Top_{\ul r} \ul b$, and $0$ otherwise.  
...
This motivates us to make the following definition, which will be used in section \ref{sec:res_altcompute}.
\begin{definition*}
...
It's interesting to note that the Bubble Lemma is intuitively very natural.  The main difficulty is finding a clean way to handle the special cases generated by the convoluted geometry.
...
We will now derive the Caterpillar Theorem as a consequence of the Bubble Lemma.  
...
\begin{figure}[ht]
	...
	\begin{asy}
		...
		label("$\to$", (7,0));
	\end{asy}
	\caption{In the first diagram, $\ul a = \ul b = 110010$.  In the second, $\ul a' = \ul b' = 110010111$.  Closing the pastures allows us to deduce the Pasture Theorem from the Bubble Lemma.}
	\label{fig:pf_pasture}
\end{figure}
The bubble theorem and the pasture theorem are structurally very similar.  In fact, we will now show a somewhat surprising proof that the Pasture Theorem follows from the Bubble Lemma.
...
\section{Computations for the Alternating Case}
\label{sec:res_altcompute}
We now turn to using the tools developed in section \ref{sec:res_altcase} in order to compute polynomials. First, we add some structure to these special cases.

\begin{definition*}
	We say that two strings $\ul{r_1}$ and $\ul{r_2}$ \emph{agree}, or \emph{agree in $c$} if their last characters are both $c$.
\end{definition*}
\begin{definition*}
	For a binary string $\ul r$, let $\ul r'$ be the string formed by deleting the last bit of $\ul r$.
\end{definition*}
...
\begin{corollary}
	...
	\[
		...
		\begin{cases}
			1 & \text{if $T$ and $\AA_n$ agree} \\
			Q_T & \text{otherwise}
		\end{cases}.
	\]
\end{corollary}
\begin{proof}
	This is an immediate consequence of the Caterpillar Theorem and the Pasture Theorem.  Everything in the first pasture is a product of monomials by the Caterpillar Theorem.  If $T$ and $\AA_n$ agree, then the last pasture is empty and hence we obtain the first case.  Otherwise, there is a single caterpillar of color opposite to $T$, and hence after evaluating that caterpillar, we obtain some monomial times a single barbell.  This generates the final $Q_T$ term.
\end{proof}
...
\begin{proposition}
	Let $n \ge 2$ be an integer.  Suppose $\ul a, \ul b \in \BB_n$ have the same top $T = \Top_{\AA_n} \ul a = \Top_{\AA_n} \ul b$, and further that $\ul a$ and $\ul b$ agree.  Suppose $M_1 = \MM_{\AA_{n-1}} (\ul a', \ul b') \neq 0$.  Then $M = \MM_{\AA_n} (\ul a, \ul b)$ is nonzero, and the following recursion holds.
	\begin{enumerate}[(i)]
		\ii If $\AA_n$ and $T$ have the same last character $c$ (so $T \neq \varnothing$)
		\begin{itemize}
			\ii If $\ul a[n] = \ul b[n] = 1$, then $M = M_1$.
			\ii If $\ul a[n] = \ul b[n] = 0$, then $M = -f / \alpha_k$ where $f$ is the monomial in $M_1$ with the largest factor of $\alpha_k$, with any quantum numbers
		\end{itemize}
	\end{enumerate}
