...
We study a class of maps between Bott-Samelson bimodules.  
Bott-Samelson bimodules arise from the study of Soergel bimodules, as mentioned in \cite{gr4all}, in the sense that a description of the category of maps between Bott-Samelson bimodules is sufficient to understand the corresponding category for the Soergel bimodules. 
However, the morphisms of Bott-Samelson bimodules admit a combinatorial description.  In particular, \cite{gr4all} gives a presentation for the category of these morphisms in terms of planar diagrammatics (i.e. diagrams in the plane).

This paper investigates some of the properties of these diagrammatics from a combinatorial perspective.   However, in this paper, we consider only the special case of an infinite dihedral group with generators $s$ and $t$ (as opposed to an arbitrary Coxeter group).

Given a sequence consisting of the letters $s$ and $t$ of length $n$, as well as a binary string of length $n$, one can construct such a diagram using an algorithm known as Libedinsky's light leaves, as detailed in Appendix~\ref{sec:prelim_lightleave}.  
These light leaves are interesting because they form a spanning set for the maps we are interested in, as shown in \cite{gr4all}.
...
The technical description of the problem statement is quite involved.  Thusly, the preliminaries for the problem are given in Appendix~\ref{sec:prelim_background}, while the definitions used in this paper are recorded in Section~\ref{sec:prelim}.  The problem statement is given in Section~\ref{sec:probstate}.
...
\begin{definition*}
	A \emph{bubble} is a bounded face alongside with any components contained within the face.  The \emph{color} of the bubble is the color of the edges which form the bounded face.  
\end{definition*}
\begin{definition*}
...
\begin{enumerate}
	...
	\par Note that, in particular, $\barbell{barbell_blue} = \alpha_s$ follows from a straightforward computation.  Furthermore, one can check that, say, $\barbell{barbell_blue}\barbell{barbell_blue}\barbell{barbell_red}\barbell{alpha_blue} = \alpha_s^2 \alpha_t$.  
...
\begin{equation}
	...
	\label{eq:quantum}
\end{equation}
and its analogous form for $t$.  One can verify \eqref{eq:quantum} immediately follows from definitions.  This gives us a robust way to express the results of action on $\alpha_s$ or $\alpha_t$ by elements of $W$.  For example, $ststst(\alpha_s) = ststst([1]_x\alpha_s + [0]_y\alpha_t) = [7]_x\alpha_s + [6]_y\alpha_t$.
...
\begin{proof}[Proof of Proposition \ref{thm:count_maps}]
	...
	We simply check that in all eight cases, the binomial coefficients add up to give the new binomial coefficients (via Pascal's identity).  The computation is left to a diligent reader.

	To finish the case $\ell \neq n$, we note that we can treat each consecutive block of $s$'s of length $m$ as a single $s$, where there a $2^{m-1}$ ways to get a $1$ and $2^{m-1}$ ways to get a $0$.  (This follows from Proposition \ref{prop:s_top}.)  This reduces to the previous case $\ul r = \AA_n$, but with some extra factors of $2$; aggregating all of them yields the factor of $2^{\ell-n}$, as desired.
\end{proof}
...
The Bubble Lemma is the key combinatorial observation, which hinges on the fact that any two anchors with the same color have a third anchor of the other color between them.  The other two lemmas follow almost directly from it.
