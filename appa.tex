...
The reasons for the above operation are not obvious; see page 7 of \cite{ref:gr4all} for an explanation.
...
\begin{proof}[Proof of Proposition \ref{thm:count_maps}]
	...
	\begin{itemize}
		...
		{$\binom{n-2}{(m-1)+k} + \binom{n-2}{m+k} = \binom{n-1}{m+k}$}
	\end{itemize}
	The case where $n=2k$ is similar, but $n-1=2(k-1)+1$.  Now the last bit changes the top by $t$.
	\begin{itemize}
